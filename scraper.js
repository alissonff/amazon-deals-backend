...
async function scrapeProduct(url) {
  ...
  const productDetails = await page.evaluate((founSelector) => {
    ...
    if (founSelector === ".a-price-whole") {
      ...
      let priceFractionElement =
        document.getElementsByClassName("a-price-fraction");
      let priceWhole =
        priceWholeElement.length > 0
          ? priceWholeElement[0].textContent.trim()
          : null;
      let priceFraction =
        priceFractionElement.length > 0
          ? priceFractionElement[0].textContent.trim()
          : null;
      ...
      let priceOneTime =
        priceOneTimeElement.length > 0
          ? priceOneTimeElement[0].textContent.trim()
          : null;
    }

    let rating = document.querySelector(".a-icon-alt")?.textContent?.trim();
    let numberOfReviews = document
